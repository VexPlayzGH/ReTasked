...
fn handle_help(key: KeyEvent, app_state: &mut AppState) {
    match key.code {
        event::KeyCode::Esc => {
...
fn get_prev_list(app_state: &mut AppState) -> Result<()> {
        ...
        let list = List::new(app_state.items
            ...
            .map(|x| {
                let value = if x.is_done {
                   x.description.to_span().add_modifier(Modifier::BOLD).style(Style::default().fg(Color::Rgb(255, 0, 0))).crossed_out()
                } else {
